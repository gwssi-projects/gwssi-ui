...
export default {
  ...
  watch: {
    $route: {
      handler: function() {
        let index = gwTools.hasValInArrayObj(
          this.nav,
          "path",
          this.$route.path
        );
        ...
        var t = this.$route.meta.title;

        // debugger

        //需要获取菜单名称而不是路由的title 遍历对比一下menu的菜单
        // for (let i = 0; i < menu.length; i++) {
        //   var subMemu = menu[i].children;

        //   for (let j = 0; j < subMemu.length; j++) {
        //     var subMemu2 = subMemu[j].children;
        //   }
        // }
